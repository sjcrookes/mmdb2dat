...
const uint32_t COUNTRY_BEGIN = 16776960;
const uint8_t COUNTRY_EDITION = 1;
const uint32_t STANDARD_RECORD_LENGTH = 1;
const uint32_t SEEK_DEPTH = 31;
const uint32_t SEGMENT_RECORD_LENGTH = 1;

const std::array< std::string, 255 > COUNTRY_CODES = {
    "",
...
const std::unordered_map< std::string, std::string > REMAP_CONTINENT_CODES{
    { "AS", "AP" }, // Asia -> Asia Pacific
};

const std::unordered_map< std::string, std::string > REMAP_COUNTRY_CODES{
    { "CW", "AN" }, // Curacao -> Netherlands Antilles
    { "UK", "GB" }, // UK -> Great Britain
    { "SX", "FX" }, // Island of Saint Martin -> French Metropolitan
    { "XK", "RS" }, // Kosovo -> Serbia
};
